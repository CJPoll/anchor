defmodule Anchor.Check.StructGetterConvention do
  ...
  defp validate_getter({_, meta, [{function_name, _, [arg]}, [do: body]]}, _struct_fields) do
    case analyze_getter_pattern(arg, body) do
      {:getter, field_name} ->
        if function_name != field_name do
          [create_naming_violation(function_name, field_name, meta)]
        else
          []
        end
  ...
  defp create_naming_violation(function_name, field_name, meta) do
    %Violation{
      message:
        "Getter function `#{function_name}` should be named `#{field_name}` to match the field it extracts",
      line: meta[:line],
